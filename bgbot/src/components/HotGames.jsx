...
const HotGames = () => {
    ...
    const FetchHotGames = async () => {
        try {
            const res = await fetch('http://localhost:8080/https://www.boardgamegeek.com/xmlapi2/hot?boardgame');
            ...
            const gamesList = Array.from(items).map((item, index) => ({
                ...
                thumbnail: item.getElementsByTagName('thumbnail')[0].getAttribute('value'),
            }));
        ...
        <div className="games-container">
            {hotGames.map((game, index) => (
                <div key={index} className="game-card">
                    ...
                    <h6 className="game-name">{game.name} (Published In: {game.yearpublished})</h6>
